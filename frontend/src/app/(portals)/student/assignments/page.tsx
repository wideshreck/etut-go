...
export default function StudentAssignmentsPage() {
  ...
  useEffect(() => {
    api
      .get<Assignment[]>("/api/v1/student/assignments")
  ...
  const filtered = useMemo(() => {
    ...
    return assignments.filter((a) => {
      if (tab === "pending")
        return !a.is_completed && new Date(a.due_date) >= now;
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-foreground text-2xl font-semibold">Ödevlerim</h1>
      ...
      <Tabs value={tab} onValueChange={(v) => setTab(v as FilterTab)}>
        <TabsList className="w-full sm:w-auto">
          ...
          <TabsTrigger value="overdue" className="flex-1 sm:flex-initial">
            Süresi Geçmiş
          </TabsTrigger>
        </TabsList>
